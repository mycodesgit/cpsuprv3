<script>
        ...
        var dataTable = $('#pendingUserTable').DataTable({
            ...
            "columns": [
                ...
                },
                {
                    ...
                    render: function(data, type, row) {
                        if (type === 'display') {
                            var buttons = '<button type="button" class="btn btn-sm btn-success btn-prpdfchecking mr-1 text-light" data-id="' + row.pid + '"  data-bs-toggle="tooltip" data-placement="top" title="View PR."><i class="fas fa-file-pdf"></i></button>';
                                if (isChecker) {
    ...
    $(document).on('click', '.btn-prpdfchecking', function () {
        ...
        $.ajax({
            url: pendingUserListViewRoute + '/' +pid,
            type: 'GET',
